...
# When done:
# 1) Set region randomly (now it's only East Canada)
# 2) Google Directions set API (now static so that we don't get charged)
# 3) Xrisimopoiise gia tis pithanotites:
# 	import numpy
...
def setup_user_profil(categories, regions):
	...
	if isWorking: 
		#with neo4j_lock:
		(workPlace, _) = my_neo4j_driver.neo4j_find_random_poi(regions, region)
...
def threadCode(iterations, user_no_start, final_categories, regions, input_consts, time_period, json_file):
	user_no = user_no_start
	while user_no < user_no_start+iterations:
...
def data_generator():
	...
	NumberOfThreads = 1
	if NumberOfThreads > NumberOfUsers:
		print("NumberOfThreads > NumberOfUsers. No point in that.")
	while True:
	...
	iterations = NumberOfUsers // NumberOfThreads

	with open('dataset_threads.json', 'w', encoding='utf-8') as json_file:
		slave = {}
		for thread_no in range(NumberOfThreads):
			print("Going to set the %s thread" % thread_no)
			user_no_start = thread_no * iterations
			slave[thread_no] = threading.Thread(target=threadCode, args=[iterations, user_no_start, final_categories, regions, input_consts, time_period, json_file])
			# slave[thread_no].daemon = True
			slave[thread_no].start()

		for thread_no in range(NumberOfThreads):
			slave[thread_no].join()
	print("Done")
